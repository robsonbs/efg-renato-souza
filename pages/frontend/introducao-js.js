...
function Introducao() {
  return (<>
    ...
    <p> A diferença entre as duas é que const é usando para algo "constante", que não muda e let para algo que pode mudar durante a execução. Curiosidade: let vem da expressão "let it change"(Deixe isso mudar).
    </p>
    <h2>Tipos de dados</h2>
    <h3>Texto (String)</h3>
    <p> Existem, basicamente, três maneiras de criar textos em JS:
    </p>
    <ul>
      <li>Utilizando aspas simples (') como em 'Olá, mundo!';</li>
      <li>Utilizando aspas duplas (") como em "Olá, mundo!";</li>
      <li>Utilizando o acento grave(`) como em `{`Boa tarde, \${nome}`}`;</li>
    </ul>

    <h3>Inteiro (Integer)</h3>
    <p>São números que não tem partes quebradas, como por exemplo quantidade de pessoas que cabem em um local, temperaturas. Podem ser positivos ou negativos. Por exemplo, 206(pessoas),-4(°C)</p>

    <h3>Decimais (Float)</h3>
    <p>São números que podem ter valores após a virgula, como preços, pesos, distâncias: (R$)9.99, 62.5(KG), 13.3(KM). </p>

    <h3>Boleano (Boolean)</h3>
    <p>É o tipo que indica uma valor lógico: verdadeiro ou falso;</p>

    <h3>Undefined</h3>
    <p>Algo não definido ainda, normalmente indica um erro no uso da variável.</p>

    <h3>Null</h3>
    <p>Variável não instânciada, ou vazia.</p>

    <h3>Vetor/Matriz (Array)</h3>
    <p> Permite guardar conjuntos de valores, como um conjunto de valores. Ex.: Temperaturas dos últimos dias.</p>
